...
AIに対するフィールドの情報は以下の形式で与えられる。
    ...
    START
    ターン番号t 最大ターン番号m プレイヤーのIDp
    フィールドの幅r タイルの数n
    タイル1のx座標 y座標 所有者のID ロボットの数 建物
    タイル2のx座標 y座標 所有者のID ロボットの数 建物
    ...
    タイルnのx座標 y座標 所有者のID ロボットの数 建物
    EOS
...
なお、以下のようにfinishを複数回出力した場合は、次のターンのコマンド内容が3体動かすコマンドになる。

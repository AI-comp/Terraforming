...
* __大規模ロボット製造機 (Initial)__ (建造不可)

  ターン開始時にロボットを5体生成する。

* __ロボット製造機 (Robot Maker)__ 資源 4 / ロボット 25
...
* __攻撃塔 (Attack)__ 資源 5 / ロボット 25
  ...
  攻撃した各マスについて、敵ロボットの数を3減らす。

* __シェルター (Shelter)__ 資源 6 / ロボット 25

  各マスのロボット数はターン終了時に、(そのマスから距離2以内のシールドの個数+1)倍になり、
  開始時に1/(そのマスから距離2以内のシールドの個数+1)になる。
  結果として、相手の戦闘、攻撃塔によって減るロボット数が
　1/(周囲2マスのシールドの個数+1)になる。
  
#### 資源系

* __掘削機 (Excavator)__ 資源 5 / ロボット 20

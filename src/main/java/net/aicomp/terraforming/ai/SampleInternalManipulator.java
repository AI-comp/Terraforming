...
public class SampleInternalManipulator implements InternalManipulator {
  ...
  public List<String> run(String stringfied, net.aicomp.terraforming.entity.Game scalaGame,
    ...
    if (rand.nextInt(1) == 0) {
      ...
      for (Point point : points) {
        if (rand.nextInt(1) == 0) {
          ...
          result.add(move(point, Direction.values()[dirIndex], rand.nextInt(tile.robot) + 1));
        }
      }
  ...
  private static String build(Point point, Installation inst) {
    return "build " + point.x + " " + point.y + " " + inst.name().toLowerCase();
  }
